...
public class CloudantRecipeStore implements RecipeStore{
	
	private Database db = null;
	private static final String databaseName = "mydb";
	
	public CloudantRecipeStore(){
	...
	public Collection<Recipe> getAll(){
        List<Recipe> docs;
		try {
			docs = db.getAllDocsRequestBuilder().includeDocs(true).build().getResponse().getDocsAs(Recipe.class);
		} catch (IOException e) {
			return null;
		}
        return docs;
	...
	public Recipe get(String id) {
		return db.find(Recipe.class, id);
	}

	@Override
	public Recipe persist(Recipe td) {
	...
	public Recipe update(String id, Recipe newRecipe) {
		Recipe recipe = db.find(Recipe.class, id);
